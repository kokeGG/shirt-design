...
export const captureFullPreview = async (item, index) => {
  ...
  // FRENTE
  if (item.frontComposite) {
    const img = await loadImage(item.frontComposite);
  ...
  // ATRÁS
  if (item.backComposite) {
    const img = await loadImage(item.backComposite);
    ctx.drawImage(img, 450, 50, 300, 400);
  ...
  ctx.fillText(
    `Playera #${index + 1} - ${item.color === 'white' ? 'Blanca' : 'Negra'}`,
    400,
    520
  );

  ctx.font = '18px Arial';
  ctx.fillText(`Precio: $${item.totalPrice} MXN`, 400, 550);
